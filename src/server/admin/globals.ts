...
export const globals: Record<GlobalSlug, GlobalConfig> = {
  'site-settings': {
    ...
    fields: [
      ...
      { name: 'hours', label: 'Hours (JSON: [{days,time}])', type: 'json' },
  ...
  'home-page': {
    ...
    fields: [
      ...
      { name: 'trustBadges', label: 'Trust bar (JSON: [{label}])', type: 'json' },
      { name: 'whyUs', label: 'Why-us (JSON: [{title,description}])', type: 'json' },
