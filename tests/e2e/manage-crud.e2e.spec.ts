...
test.describe('Custom admin CRUD (/manage)', () => {
  test('create, edit, and delete a review', async ({ page }) => {
    ...
    // Reviews list
    await page.locator('a[href="/manage/reviews"]').click()
    await expect(page).toHaveURL(/\/manage\/reviews$/)

    // Create
    await page.locator('a[href="/manage/reviews/new"]').click()
